...
export class TripService {
  ...
  protected convertDateFromClient(trip: ITrip): ITrip {
    const copy: ITrip = Object.assign({}, trip, {
      ...
      lastModified: trip.lastModified && trip.lastModified.isValid() ? trip.lastModified.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((trip: ITrip) => {
        ...
        trip.lastModified = trip.lastModified ? moment(trip.lastModified) : undefined;
      });
    }
    return res;
  }
