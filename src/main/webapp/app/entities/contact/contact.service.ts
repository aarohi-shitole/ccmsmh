...
export class ContactService {
  ...
  protected convertDateFromClient(contact: IContact): IContact {
    const copy: IContact = Object.assign({}, contact, {
      lastModified: contact.lastModified && contact.lastModified.isValid() ? contact.lastModified.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((contact: IContact) => {
        contact.lastModified = contact.lastModified ? moment(contact.lastModified) : undefined;
      });
    }
    return res;
  }
