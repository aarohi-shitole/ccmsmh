...
export class StateService {
  ...
  protected convertDateFromClient(state: IState): IState {
    const copy: IState = Object.assign({}, state, {
      lastModified: state.lastModified && state.lastModified.isValid() ? state.lastModified.toJSON() : undefined,
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((state: IState) => {
        state.lastModified = state.lastModified ? moment(state.lastModified) : undefined;
      });
    }
    return res;
  }
